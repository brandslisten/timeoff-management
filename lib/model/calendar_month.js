...
function CalendarMonth(day, args){
    ...
    if (args && args.leave_days){
        ...
        args.leave_days.forEach(function(day){
            ...
            if ( ! map[attribute] ) {
                map[attribute] = day;
                map[attribute]['leave_days'] = [ day ];
            } else if ( map[attribute] ) {
                map[attribute]['leave_days'].push( day );
...
CalendarMonth.prototype.is_holiday = function(leaveDay) {
  if (leaveDay.leave_obj) {
    return leaveDay.leave_obj.dataValues.leaveTypeId === 3;
  } else {
    return false;
  }
}

CalendarMonth.prototype.is_homeOffice = function(leaveDay) {
  if (leaveDay.leave_obj) {
    return leaveDay.leave_obj.dataValues.leaveTypeId === 4;
  } else {
    return false;
  }
}

CalendarMonth.prototype.is_ill = function(leaveDay) {
  if (leaveDay.leave_obj) {
    return leaveDay.leave_obj.dataValues.leaveTypeId === 5;
  } else {
    return false;
  }
}

CalendarMonth.prototype.is_offsite = function(leaveDay) {
  if (leaveDay.leave_obj) {
    return leaveDay.leave_obj.dataValues.leaveTypeId === 6;
  } else {
    return false;
  }
}
...
CalendarMonth.prototype.as_for_template = function(){
    ...
    for( var i=1; i<=this.how_many_days(); i++){
        ...
        if ( this.is_holiday(day) ) {
          day.is_holiday = true;
        }

        if ( this.is_homeOffice(day) ) {
          day.is_home_office = true;
        }
